...
namespace DALayer
{
    public class DALModelInfo
    {
        ...
        {
            string SqlQuery = @"INSERT INTO T_Module ([ModuleName]  ,[FormName] ,[ParentID])
                              VALUES('" + paramModeName + "','" + paramFormName + "','" + paramParentID + "')";
            int rows = 0;
        ...
        public int UpdateModuleInfo(int paramID, string paramModeName, string paramFormName, string paramParentID)
        {
            string SqlQuery = @"UPDATE  T_Module set  ModuleName='" + paramModeName
                                              ...
                                              +"' ,ParentID='" +paramParentID+"'   where ID='"+paramID+"'    ";
            int rows = 0;
            rows = mySqlconn.ExecuteNonQuery(SqlQuery);
            return rows;
